...
constexpr lv_coord_t ITEM_WIDTH = 22;
constexpr lv_coord_t ITEM_HEIGHT = 18;
constexpr lv_coord_t SELECTION_CURSOR_WIDTH = 8;
constexpr lv_coord_t SELECTION_CURSOR_HEIGHT = 2;
constexpr lv_coord_t SELECTION_CURSOR_OFFSET_Y = 2;
constexpr lv_coord_t HORIZONTAL_INSET = theme::layout::MARGIN_SM + 4;

constexpr uint32_t ACTIVE_COLOR = theme::color::TEXT_PRIMARY;
constexpr lv_opa_t HIGHLIGHTED_OPA = LV_OPA_COVER;
constexpr lv_opa_t ACTIVE_OPA = LV_OPA_70;
constexpr lv_opa_t INACTIVE_OPA = LV_OPA_50;
...
void StepPropertyStrip::createUI(lv_obj_t* parent) {
    ...
    lv_obj_set_style_pad_top(container_, theme::layout::MARGIN_XS, 0);
...
void StepPropertyStrip::render(const StepPropertyStripProps& props) {
    ...
    const int selectedIndex = std::clamp(props.selectedIndex, 0, static_cast<int>(STRIP_PROPERTIES.size()) - 1);
    const size_t activeIndex = propertyStripIndex(props.activeProperty);
    const size_t highlightedIndex = props.selecting
    ...
    lv_obj_set_pos(
        selection_cursor_,
        static_cast<lv_coord_t>(lv_obj_get_x(highlightedItem) + (ITEM_WIDTH - SELECTION_CURSOR_WIDTH) / 2),
        static_cast<lv_coord_t>(lv_obj_get_y(highlightedItem) + ITEM_HEIGHT + SELECTION_CURSOR_OFFSET_Y)
    );
}
