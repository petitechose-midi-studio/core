...
FLASHMEM void
SequencerStepEditHandler::openStepPresetLibrary() {
    ...
    (void)preset_library_.open(
        step_preset_library_adapter_.operations()
    );
}

FLASHMEM void
SequencerStepEditHandler::openChordPresetLibrary() {
    if (!chordEditorActive() ||
        !sequencer_.stepContentDraft.active.get() ||
        sequencer_.stepContentDraft.exitPromptVisible.get()) {
        return;
    }
    const auto subEditor =
        sequencer_.stepEdit.chordEditor.subEditor.get();
    ...
    preset_library_auto_close_at_ms_ = 0U;
    (void)preset_library_.open(
        chord_preset_library_adapter_.operations()
    );
}

FLASHMEM void
SequencerStepEditHandler::closePresetLibrary() {
...
FLASHMEM void
SequencerStepEditHandler::backFromPresetLibrary() {
...
FLASHMEM void
SequencerStepEditHandler::movePresetLibraryItem(float delta) {
    if (preset_library_auto_close_pending_) return;
    preset_library_.move(delta, time_provider_());
}

FLASHMEM void
SequencerStepEditHandler::adjustPresetLibraryDetail(
    float delta
) {
    if (preset_library_auto_close_pending_) return;
...
FLASHMEM void
SequencerStepEditHandler::enterPresetLibraryDetail() {
    if (preset_library_auto_close_pending_) return;
    preset_library_.enterDetail();
}

FLASHMEM void
SequencerStepEditHandler::togglePresetLibraryMode() {
    if (preset_library_auto_close_pending_) return;
    preset_library_.toggleMode();
}

FLASHMEM void
SequencerStepEditHandler::beginPresetLibraryActionGuard() {
    if (preset_library_auto_close_pending_) return;
    preset_library_action_press_active_ =
        preset_library_.beginActionGuard(time_provider_());
}

FLASHMEM void
SequencerStepEditHandler::releasePresetLibraryAction() {
    if (preset_library_action_press_active_) {
        preset_library_action_press_active_ = false;
        (void)preset_library_.cancelActionGuard(
            time_provider_()
        );
        return;
    }
    ...
    handlePresetLibraryResult(
        preset_library_.executeTap(time_provider_())
    );
}

FLASHMEM void
SequencerStepEditHandler::commitPresetLibraryActionGuard() {
    if (preset_library_auto_close_pending_) return;
    ...
    handlePresetLibraryResult(
        preset_library_.commitActionGuard(time_provider_())
    );
}

FLASHMEM void
SequencerStepEditHandler::handlePresetLibraryResult(
    ...
    if (result.refreshPublishedState) {
    ...
    preset_library_auto_close_at_ms_ =
        ...
        (result.outcome ==
                 SequencerPresetLibraryOutcome::CANCELLED
             ? Config::Timing::
                   CONTEXT_CANCELLED_FEEDBACK_MS
             : Config::Timing::
                   CONTEXT_APPLIED_FEEDBACK_MS);
