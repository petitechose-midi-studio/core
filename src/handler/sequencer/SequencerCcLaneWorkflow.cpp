...
contextual::ContextActionVariant variant(ActionId action, ActionImpact impact,
                                         Availability availability, Reason reason, Icon icon,
                                         Tone tone) {
    return {
        .action = action,
        .impact = impact,
...
FLASHMEM SequencerCcLaneWorkflow::SequencerCcLaneWorkflow(StateRefs state,
                                                          SequencerCcLaneDomainServices services)
    : editor_(state.editor), tracks_(state.tracks), project_navigation_(state.projectNavigation),
      history_(state.history), status_bar_(state.statusBar),
      midi_cc_coordinator_(state.midiCcCoordinator), services_(services),
...
FLASHMEM bool SequencerCcLaneWorkflow::selectorFocusesAdd() const {
    return selectorLane() < 0 && editor_.ccLaneUi.selectorIndex + 1U == selectorItemCount();
}

FLASHMEM void SequencerCcLaneWorkflow::openLaneSelector() {
    if (!commitEventEdit(0)) return;
...
FLASHMEM void SequencerCcLaneWorkflow::suspendGridForPropertySelector(uint32_t nowMs) {
...
FLASHMEM void SequencerCcLaneWorkflow::closeOneLevel(uint32_t nowMs) {
    ...
    switch (ui.mode) {
        ...
        default: ui.mode = seq::SequencerCcLaneUiMode::CLOSED; break;
    }
    refreshProjection();
}
...
FLASHMEM bool SequencerCcLaneWorkflow::createDefaultLane(uint32_t nowMs) {
    ...
    draft.destination.controller = core::state::project::sanitizeProjectCcLaneDefault(
        project_navigation_.ccLaneDefaultControllers[ui.focusedLane], ui.focusedLane);
    ...
    draft.destination.pinnedChannel = services_.trackRoute(tracks_.activeTrackIndex()).channel;
    draft.initialValue = 64U;

    const auto preflight = services_.preflight(tracks_.activeTrackIndex(), ui.focusedLane, draft);
    ...
    publishFeedback_(ActionId::CREATE, contextual::OperationFeedbackStatus::APPLIED,
                     preflight.macroConflict
                         ? Reason::CONFLICT
                         : (preflight.routeValid ? Reason::NONE : Reason::NO_ROUTE),
                     contextual::OperationFeedbackExpiryPolicy::AFTER_DURATION, nowMs, 900);
    return true;
}
...
FLASHMEM bool SequencerCcLaneWorkflow::openSettings() {
    if (!commitEventEdit(0)) return false;
    auto& ui = editor_.ccLaneUi;
...
FLASHMEM void SequencerCcLaneWorkflow::moveDraftField(float delta) {
    ...
    const auto layout =
        seq::buildSequencerCcLaneDraftLayout(ui.draft.destination.routePolicy, ui.advancedSettings);
...
FLASHMEM void SequencerCcLaneWorkflow::editDraft(float delta) {
    ...
    switch (ui.focusedField) {
        ...
        case seq::SequencerCcLaneDraftField::ROUTE_POLICY:
            destination.routePolicy =
                destination.routePolicy == seq::SequencerCcLaneRoutePolicy::INHERIT_TRACK
            ...
            if (destination.routePolicy == seq::SequencerCcLaneRoutePolicy::PINNED) {
                ...
                destination.pinnedChannel =
                    services_.trackRoute(tracks_.activeTrackIndex()).channel;
            }
            break;
        case seq::SequencerCcLaneDraftField::PINNED_CHANNEL:
            if (destination.routePolicy == seq::SequencerCcLaneRoutePolicy::PINNED) {
                destination.pinnedChannel = clampAdd(destination.pinnedChannel, direction, 0, 15);
            }
            break;
        case seq::SequencerCcLaneDraftField::MINIMUM:
            destination.minimum = clampAdd(destination.minimum, direction, 0, destination.maximum);
            ui.draft.initialValue = std::max(ui.draft.initialValue, destination.minimum);
            break;
        case seq::SequencerCcLaneDraftField::MAXIMUM:
            destination.maximum =
                clampAdd(destination.maximum, direction, destination.minimum, 127);
            ui.draft.initialValue = std::min(ui.draft.initialValue, destination.maximum);
            break;
        case seq::SequencerCcLaneDraftField::INITIAL:
            ui.draft.initialValue = clampAdd(ui.draft.initialValue, direction, destination.minimum,
                                             destination.maximum);
            break;
        case seq::SequencerCcLaneDraftField::ADVANCED: return;
...
FLASHMEM bool SequencerCcLaneWorkflow::focusStep(uint8_t step, uint32_t nowMs) {
    ...
    editor_.page.set(static_cast<uint8_t>(step / seq::SequencerPatternState::STEPS_PER_PAGE));
    refreshProjection();
    return true;
}
...
FLASHMEM SequencerCcLaneWorkflow::PatternChangePtr SequencerCcLaneWorkflow::prepareChange_(
    seq::SequencerHistoryActionKind kind, uint8_t lane, uint8_t step) {
...
FLASHMEM bool SequencerCcLaneWorkflow::applySettings_(bool macroConflictAuthorized,
                                                      uint32_t nowMs) {
    auto& ui = editor_.ccLaneUi;
    if (ui.mode != seq::SequencerCcLaneUiMode::LANE_SETTINGS) return false;
    const auto preflight =
        services_.preflight(tracks_.activeTrackIndex(), ui.focusedLane, ui.draft);
    if (preflight.laneConflict) {
    ...
    const auto mutation = seq::updateSequencerCcLaneSettings(*staged, ui.focusedLane, draft);
    if (!mutation.changed()) {
    ...
    ui.acceptedMacroConflict = draft.acceptedMacroConflict;
    openGrid_(ui.focusedLane);
    publishFeedback_(ActionId::APPLY, contextual::OperationFeedbackStatus::APPLIED,
                     preflight.routeValid ? Reason::NONE : Reason::NO_ROUTE,
                     contextual::OperationFeedbackExpiryPolicy::AFTER_DURATION, nowMs, 900);
    return true;
}
...
FLASHMEM bool SequencerCcLaneWorkflow::focusVisibleStep_(uint8_t indexInWindow, uint32_t nowMs) {
...
FLASHMEM bool SequencerCcLaneWorkflow::editVisibleEvent(uint8_t indexInWindow, float normalized,
    ...
    const uint8_t value = static_cast<uint8_t>(std::lround(
        ...
        clamped * static_cast<float>(lane.destination.maximum - lane.destination.minimum)));
    return setFocusedEventValue_(value, nowMs);
}

FLASHMEM bool SequencerCcLaneWorkflow::toggleVisibleEvent(uint8_t indexInWindow, uint32_t nowMs) {
    return focusVisibleStep_(indexInWindow, nowMs) && toggleFocusedEvent(nowMs);
}

FLASHMEM bool SequencerCcLaneWorkflow::openTransitionPicker(uint8_t indexInWindow, uint32_t nowMs) {
    if (!focusVisibleStep_(indexInWindow, nowMs)) return false;
    return openTransitionPickerForFocused_(false, nowMs);
}

FLASHMEM bool SequencerCcLaneWorkflow::openFocusedTransitionPicker(uint32_t nowMs) {
    return openTransitionPickerForFocused_(true, nowMs);
}

FLASHMEM bool SequencerCcLaneWorkflow::openTransitionPickerForFocused_(bool compact,
                                                                       uint32_t nowMs) {
    auto& ui = editor_.ccLaneUi;
...
FLASHMEM void SequencerCcLaneWorkflow::moveTransition(float delta) {
    ...
    ui.selectedTransition =
        static_cast<seq::SequencerCcLaneTransition>((current + direction + COUNT) % COUNT);
    ui.bump();
}

FLASHMEM bool SequencerCcLaneWorkflow::selectTransitionNormalized(float normalized) {
    auto& ui = editor_.ccLaneUi;
    if (ui.mode != seq::SequencerCcLaneUiMode::TRANSITION_PICKER || !std::isfinite(normalized)) {
        return false;
    }
    constexpr int COUNT = 5;
    const int selected =
        std::clamp(static_cast<int>(std::lround(std::clamp(normalized, 0.0f, 1.0f) *
...
FLASHMEM bool SequencerCcLaneWorkflow::clearFocusedEvent_(uint32_t nowMs) {
    ...
    if (!seq::clearSequencerCcLaneEvent(*staged, ui.focusedLane, ui.focusedStep).changed())
        return false;
    ...
    publishFeedback_(ActionId::CLEAR, contextual::OperationFeedbackStatus::APPLIED, Reason::NONE,
                     contextual::OperationFeedbackExpiryPolicy::AFTER_DURATION, nowMs, 650);
    refreshProjection();
    return true;
}
...
FLASHMEM bool SequencerCcLaneWorkflow::deleteCurrentLane_(uint32_t nowMs) {
    ...
    ui.mode = seq::SequencerCcLaneUiMode::CLOSED;
    ui.selectorIndex = 0;
    publishFeedback_(ActionId::REMOVE, contextual::OperationFeedbackStatus::APPLIED, Reason::NONE,
                     contextual::OperationFeedbackExpiryPolicy::AFTER_DURATION, nowMs, 900);
    refreshProjection();
    return true;
}

FLASHMEM bool SequencerCcLaneWorkflow::executeTap(seq::SequencerCcLaneActionSlot slot,
                                                  uint32_t nowMs) {
    ...
    switch (slot) {
        case seq::SequencerCcLaneActionSlot::BOTTOM_LEFT: return clearFocusedEvent_(nowMs);
        case seq::SequencerCcLaneActionSlot::BOTTOM_CENTER: return false;
        case seq::SequencerCcLaneActionSlot::BOTTOM_RIGHT:
            return editor_.ccLaneUi.mode == seq::SequencerCcLaneUiMode::LANE_GRID
                       ? openSettings()
                       : applySettings_(false, nowMs);
        case seq::SequencerCcLaneActionSlot::COUNT: return false;
    }
    return false;
}

FLASHMEM bool SequencerCcLaneWorkflow::beginGuard(seq::SequencerCcLaneActionSlot slot,
                                                  uint32_t nowMs) {
    const auto spec = editor_.ccLaneUi.action(slot);
    if (!contextual::canExecute(spec.hold) || !contextual::requiresGuard(spec)) return false;
    auto guard = editor_.ccLaneUi.actionGuard.get();
    contextual::resetGuardedAction(guard);
    if (!contextual::beginGuardedActionPress(guard, nowMs, spec.guard.durationMs)) { return false; }
    guard_slot_ = slot;
    editor_.ccLaneUi.actionGuard.set(guard);
    publishFeedback_(spec.hold.action, contextual::OperationFeedbackStatus::PRESSED,
                     spec.hold.reason, contextual::OperationFeedbackExpiryPolicy::MANUAL, nowMs);
    return true;
}

FLASHMEM bool SequencerCcLaneWorkflow::releaseGuard(seq::SequencerCcLaneActionSlot slot,
                                                    uint32_t nowMs) {
    auto guard = editor_.ccLaneUi.actionGuard.get();
    ...
    if (guard.phase == contextual::GuardedActionPhase::PRESSED &&
        static_cast<uint32_t>(nowMs - guard.pressedAtMs) >= Config::Timing::LATCH_THRESHOLD_MS) {
    ...
    if (release == contextual::GuardedActionRelease::TAP) {
        ...
        if (!contextual::hasTapAction(spec)) {
            publishFeedback_(spec.hold.action, contextual::OperationFeedbackStatus::CANCELLED,
                             Reason::NO_ACTION,
                             contextual::OperationFeedbackExpiryPolicy::AFTER_DURATION, nowMs, 650);
            return false;
        }
    ...
    if (release != contextual::GuardedActionRelease::COMMITTED) {
        publishFeedback_(editor_.ccLaneUi.action(slot).hold.action,
                         contextual::OperationFeedbackStatus::CANCELLED, Reason::NONE,
                         contextual::OperationFeedbackExpiryPolicy::AFTER_DURATION, nowMs, 650);
        return false;
    }
...
FLASHMEM void SequencerCcLaneWorkflow::publishFeedback_(
    ActionId action, contextual::OperationFeedbackStatus status, Reason reason,
    ...
    contextual::setOperationFeedback(
        feedback, action, laneEntity(tracks_.activeTrackIndex(), editor_.ccLaneUi.focusedLane),
        laneEntity(tracks_.activeTrackIndex(), editor_.ccLaneUi.focusedLane), status, reason,
        expiry, nowMs, durationMs);
    editor_.ccLaneUi.operationFeedback.set(feedback);
}

FLASHMEM void SequencerCcLaneWorkflow::block_(ActionId action, Reason reason, uint32_t nowMs) {
    publishFeedback_(action, contextual::OperationFeedbackStatus::BLOCKED, reason,
                     contextual::OperationFeedbackExpiryPolicy::AFTER_DURATION, nowMs, 1100);
}
...
FLASHMEM void SequencerCcLaneWorkflow::refreshValueProjection_() {
    ...
    if (ui.mode != seq::SequencerCcLaneUiMode::LANE_GRID || !status_bar_.playing.get()) { return; }
    const auto live = projectSequencerCcLaneLive(midi_cc_coordinator_,
                                                 {tracks_.activeTrackIndex(), ui.focusedLane}, lane,
...
FLASHMEM void SequencerCcLaneWorkflow::refreshActions_(const SequencerCcLanePreflight& preflight) {
    auto& ui = editor_.ccLaneUi;
    ui.actions = {};
    ...
    if (ui.mode == seq::SequencerCcLaneUiMode::LANE_SELECTOR ||
        ...
        const bool laneAvailable =
            ui.mode == seq::SequencerCcLaneUiMode::LANE_GRID || selectorLane() >= 0;
        settings.tap = variant(ActionId::OPEN_SETTINGS, ActionImpact::NON_MUTATING,
                               laneAvailable ? Availability::AVAILABLE : Availability::DISABLED,
                               laneAvailable ? Reason::NONE : Reason::EMPTY_SELECTION, Icon::EDIT,
                               Tone::NEUTRAL);
    }
    if (ui.mode == seq::SequencerCcLaneUiMode::LANE_GRID) {
        auto& clear = makeSpec(seq::SequencerCcLaneActionSlot::BOTTOM_LEFT);
        clear.tap = variant(ActionId::CLEAR, ActionImpact::VALUE_EDIT,
                            ui.hasAuthoredValue ? Availability::AVAILABLE : Availability::DISABLED,
                            ui.hasAuthoredValue ? Reason::NONE : Reason::NO_ACTION, Icon::CLEAR,
                            Tone::NEUTRAL);
    }
    if (ui.mode == seq::SequencerCcLaneUiMode::LANE_SETTINGS) {
        ...
        if (preflight.laneConflict) {
            apply.tap = variant(action, ActionImpact::CONSTRUCTIVE, Availability::DISABLED,
                                Reason::CONFLICT, Icon::CONFLICT, Tone::AMBER);
            apply.hold = apply.tap;
        } else if (preflight.macroConflict) {
            apply.tap = variant(action, ActionImpact::CONSTRUCTIVE, Availability::DISABLED,
                                Reason::CONFLICT, Icon::CONFLICT, Tone::AMBER);
            apply.hold = variant(action, ActionImpact::CONSTRUCTIVE, Availability::WARNING,
                                 Reason::CONFLICT, Icon::HOLD, Tone::AMBER);
            apply.guard = {
            ...
            apply.tap = variant(action, ActionImpact::CONSTRUCTIVE,
                                dirtyEnough ? (preflight.routeValid ? Availability::AVAILABLE
                                                                    : Availability::WARNING)
                                            : Availability::DISABLED,
                                dirtyEnough ? routeReason : Reason::NO_ACTION, Icon::APPLY,
                                preflight.routeValid ? Tone::GREEN : Tone::AMBER);
        }
    }
    if (ui.mode == seq::SequencerCcLaneUiMode::LANE_SETTINGS) {
        auto& remove = makeSpec(seq::SequencerCcLaneActionSlot::BOTTOM_LEFT);
        remove.hold = variant(ActionId::REMOVE, ActionImpact::DESTRUCTIVE, Availability::AVAILABLE,
                              Reason::NONE, Icon::REMOVE, Tone::RED);
        remove.guard = {
            .kind = contextual::ContextGuardKind::HOLD,
            .durationMs = seq::SequencerCcLaneUiState::ACTION_GUARD_MS,
        };
...
FLASHMEM void SequencerCcLaneWorkflow::refreshProjection() {
    ...
    if (ui.mode == seq::SequencerCcLaneUiMode::LANE_SETTINGS) {
        ...
        if (bank != nullptr && ui.focusedLane < bank->lanes.size() &&
            ...
            current.acceptedMacroConflict = bank->lanes[ui.focusedLane].acceptedMacroConflict;
            preflight = services_.preflight(tracks_.activeTrackIndex(), ui.focusedLane, current);
        }
    }
...
FLASHMEM void SequencerCcLaneWorkflow::update(uint32_t nowMs) {
    ...
    if (contextual::updateOperationFeedback(feedback, nowMs)) {
        editor_.ccLaneUi.operationFeedback.set(feedback);
        if (!feedback.active) { editor_.ccLaneUi.transitionAppliedFeedback = false; }
    }
    auto guard = editor_.ccLaneUi.actionGuard.get();
    if (guard.phase == contextual::GuardedActionPhase::PRESSED &&
        static_cast<uint32_t>(nowMs - guard.pressedAtMs) >= Config::Timing::LATCH_THRESHOLD_MS) {
        ...
        if (contextual::armGuardedAction(guard, pressedAt)) {
            editor_.ccLaneUi.actionGuard.set(guard);
            publishFeedback_(editor_.ccLaneUi.action(guard_slot_).hold.action,
                             contextual::OperationFeedbackStatus::ARMED,
