...
FLASHMEM uint8_t defaultLengthForChildKind(core::state::sequencer::StepContentChildKind childKind) {
...
FLASHMEM bool focusedRowIsContext(const core::state::sequencer::SequencerState& sequencer) {
    return step_edit_rows::isContext(sequencer.stepEdit.focusedRow.get());
}

FLASHMEM bool focusedContextHasChild(const core::state::sequencer::SequencerState& sequencer,
                                     uint8_t step) {
    if (!focusedRowIsContext(sequencer)) return false;
    return core::state::sequencer::activeContentStepHasChildContent(sequencer, step,
                                                                    focusedChildKind(sequencer));
}

FLASHMEM bool canPasteFocusedContextChild(const core::state::sequencer::SequencerState& sequencer,
                                          uint8_t step,
                                          const core::state::StructureClipboardState& clipboard) {
    if (!focusedRowIsContext(sequencer)) return false;
    const auto childKind = focusedChildKind(sequencer);
    return core::state::sequencer::clipboardCanPasteActiveContentChild(clipboard, childKind) &&
           core::state::sequencer::activeContentStepCanReceiveChildContent(sequencer, step);
}

FLASHMEM core::state::sequencer::StepContentOpenResult openOrCreateFocusedContextChild(
    core::state::sequencer::SequencerState& sequencer, uint8_t step) {
    if (!focusedRowIsContext(sequencer)) return {};
    const auto childKind = focusedChildKind(sequencer);
    return core::state::sequencer::openOrCreateActiveContentChild(
        sequencer, step, childKind, defaultLengthForChildKind(childKind));
}
...
FLASHMEM bool copyFocusedContextChildToClipboard(
    ...
    return core::state::sequencer::copyActiveContentChildToClipboard(
        sequencer, step, focusedChildKind(sequencer), clipboard);
}

FLASHMEM bool clearFocusedContextChild(core::state::sequencer::SequencerState& sequencer,
                                       uint8_t step) {
    if (!focusedContextHasChild(sequencer, step)) return false;
    return seq::clearActiveContentChildPreservingGraphOwner(sequencer, step,
                                                            focusedChildKind(sequencer));
}
...
FLASHMEM bool pasteFocusedContextChildFromClipboard(
    ...
    return seq::pasteActiveContentChildFromClipboardPreservingGraphOwner(
        sequencer, step, focusedChildKind(sequencer), clipboard);
}
