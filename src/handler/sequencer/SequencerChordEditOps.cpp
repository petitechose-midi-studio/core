...
FLASHMEM Spec semanticDefault(bool scaleBased) {
    ...
    return Spec::semantic(harmony, oc::note::sequencer::recommendedChordVoiceCount(harmony),
                          oc::note::sequencer::StepSequencerChordVoicing::Close, 0,
...
FLASHMEM bool commitSpec(core::state::sequencer::SequencerState& sequencer, uint8_t step,
                         Spec spec) {
    spec.clamp();
    const auto nodeId = core::state::sequencer::activeContentStepNodeId(sequencer, step);
    const bool changed = core::state::sequencer::setAuthoringNodeChordSpec(sequencer, nodeId, spec);
    if (changed) {
...
FLASHMEM bool captureNodeSnapshot(
    const core::state::sequencer::SequencerState& sequencer, uint16_t nodeId,
    ...
    if (node != nullptr) {
        snapshot.modePresent = node->has(oc::note::sequencer::STEP_NODE_CHORD_MODE);
        snapshot.localPresent = node->has(oc::note::sequencer::STEP_NODE_CHORD_LOCAL);
        snapshot.mode = node->chordMode;
        snapshot.spec = node->chordSpec;
    }
    const bool draftResolved = core::state::sequencer::resolveStepContentDraftChord(
        sequencer, nodeId, snapshot.modePresent, snapshot.localPresent, snapshot.mode,
        snapshot.spec);
...
FLASHMEM bool sameSnapshot(const core::state::sequencer::SequencerChordAuthoringSnapshot& lhs,
                           const core::state::sequencer::SequencerChordAuthoringSnapshot& rhs) {
    return lhs.valid == rhs.valid && lhs.nodeId == rhs.nodeId &&
           lhs.modePresent == rhs.modePresent && lhs.localPresent == rhs.localPresent &&
           lhs.mode == rhs.mode && sameSpec(lhs.spec, rhs.spec);
}
...
FLASHMEM int modeChoiceCount(bool rootContext) { return rootContext ? 2 : 3; }

FLASHMEM int modeChoiceIndex(bool rootContext, oc::note::sequencer::StepSequencerChordMode mode) {
...
FLASHMEM core::state::sequencer::SequencerChordSourceChoice sourceChoiceForMode(
    bool rootContext, oc::note::sequencer::StepSequencerChordMode mode) {
    using Choice = core::state::sequencer::SequencerChordSourceChoice;
    using ChordMode = oc::note::sequencer::StepSequencerChordMode;
    if (mode == ChordMode::Local) return Choice::LOCAL_CHORD;
    if (!rootContext && mode == ChordMode::Inherit) { return Choice::PARENT_CHORD; }
    return Choice::SINGLE_NOTE;
}

FLASHMEM int sourceChoiceCount(bool rootContext) { return rootContext ? 2 : 3; }

FLASHMEM int sourceChoiceIndex(bool rootContext,
...
FLASHMEM core::state::sequencer::SequencerChordSourceChoice sourceChoiceForIndex(bool rootContext,
                                                                                 int index) {
    using Choice = core::state::sequencer::SequencerChordSourceChoice;
    if (rootContext) { return index <= 0 ? Choice::SINGLE_NOTE : Choice::LOCAL_CHORD; }
    if (index <= 0) return Choice::PARENT_CHORD;
    if (index == 1) return Choice::SINGLE_NOTE;
    return Choice::LOCAL_CHORD;
}

FLASHMEM int quickChoiceCount(bool rootContext) { return modeChoiceCount(rootContext); }

FLASHMEM int quickChoiceIndex(const core::state::sequencer::SequencerStepChordUiState& chord) {
    return modeChoiceIndex(chord.rootContext, chord.mode);
}

FLASHMEM bool applyQuickChoice(core::state::sequencer::SequencerState& sequencer, uint8_t step,
                               int choice, bool scaleBased) {
    const auto current = core::state::sequencer::resolveStepChordUiState(sequencer, step);
    const Spec local =
        current.valid && current.mode == oc::note::sequencer::StepSequencerChordMode::Local
            ? current.spec
            : semanticDefault(scaleBased);
    return applyModeChoice(sequencer, step, choice, local, scaleBased);
}

FLASHMEM bool applyModeChoice(core::state::sequencer::SequencerState& sequencer, uint8_t step,
                              int choice, Spec specForLocal, bool scaleBased) {
    using ChordMode = oc::note::sequencer::StepSequencerChordMode;
    ...
    if (rootContext) {
        changed = choice <= 0
                      ? core::state::sequencer::clearAuthoringNodeChordState(sequencer, nodeId)
                      : core::state::sequencer::setAuthoringNodeChordSpec(sequencer, nodeId,
                                                                          specForLocal);
    } else if (choice <= 0) {
        changed = core::state::sequencer::clearAuthoringNodeChordState(sequencer, nodeId);
    } else if (choice == 1) {
        changed =
            core::state::sequencer::setAuthoringNodeChordMode(sequencer, nodeId, ChordMode::Single);
    } else {
        changed =
            core::state::sequencer::setAuthoringNodeChordSpec(sequencer, nodeId, specForLocal);
    }
...
FLASHMEM bool applySourceChoice(core::state::sequencer::SequencerState& sequencer, uint8_t step,
                                core::state::sequencer::SequencerChordSourceChoice choice,
                                const core::state::sequencer::SequencerStepChordUiState& chord) {
    return applyModeChoice(sequencer, step, sourceChoiceIndex(chord.rootContext, choice),
                           chord.spec, chord.intervalsUseScaleDegrees);
}

FLASHMEM bool createDefaultLocalChord(core::state::sequencer::SequencerState& sequencer,
                                      uint8_t step, bool scaleBased) {
    const auto nodeId = core::state::sequencer::activeContentStepNodeId(sequencer, step);
    const bool changed = core::state::sequencer::setAuthoringNodeChordSpec(
        sequencer, nodeId, semanticDefault(scaleBased));
    if (changed) {
        sequencer.invalidateVariationTelemetry();
        core::state::sequencer::notifyStepContentDraftMutation(sequencer);
    }
...
FLASHMEM bool captureAuthoringSnapshot(
    ...
    return captureNodeSnapshot(
        sequencer, core::state::sequencer::activeContentStepNodeId(sequencer, step), snapshot);
}
...
FLASHMEM bool restoreAuthoringSnapshot(
    ...
    if (!snapshot.valid ||
        snapshot.nodeId == core::state::sequencer::SequencerChordAuthoringSnapshot::INVALID_NODE) {
        return false;
    }
    ...
    bool changed = core::state::sequencer::clearAuthoringNodeChordState(sequencer, snapshot.nodeId);
    if (snapshot.localPresent) {
        changed = core::state::sequencer::setAuthoringNodeChordSpec(sequencer, snapshot.nodeId,
                                                                    snapshot.spec) ||
                  changed;
    } else if (snapshot.modePresent) {
        changed = core::state::sequencer::setAuthoringNodeChordMode(sequencer, snapshot.nodeId,
                                                                    snapshot.mode) ||
                  changed;
    }
    if (changed) {
...
FLASHMEM bool applySpecField(core::state::sequencer::SequencerState& sequencer, uint8_t step,
    ...
    switch (field) {
        case Field::SHAPE: {
            ...
            spec.setVoices(oc::note::sequencer::recommendedChordVoiceCount(harmony));
        ...
        case Field::VOICING:
            ...
            break;
        case Field::STRUM:
            spec.strum = signedFromNormalized(normalized, Spec::MIN_STRUM, Spec::MAX_STRUM);
            break;
        case Field::VELOCITY_CONTOUR:
            spec.velocityCurve = signedFromNormalized(normalized, Spec::MIN_VELOCITY_CURVE,
...
FLASHMEM bool resetSpecField(core::state::sequencer::SequencerState& sequencer, uint8_t step,
                             core::state::sequencer::SequencerChordEditField field,
                             const core::state::sequencer::SequencerStepChordUiState& chord) {
    using Field = core::state::sequencer::SequencerChordEditField;
    ...
    switch (field) {
        ...
        case Field::VOICING:
            ...
            break;
        case Field::STRUM: spec.strum = 0; break;
        case Field::VELOCITY_CONTOUR: spec.velocityCurve = 0; break;
        case Field::PITCH_CONTEXT:
        case Field::COUNT:
        default: return false;
    }
