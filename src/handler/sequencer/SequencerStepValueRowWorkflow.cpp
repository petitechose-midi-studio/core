...
FLASHMEM bool focusedRowIsValue(
    const core::state::sequencer::SequencerState& sequencer
) {
    return focusedRowIsActivated(sequencer) ||
           focusedRowIsProperty(sequencer) ||
...
FLASHMEM bool focusedRowSupportsLocalVariation(
    ...
    return core::state::sequencer::stepPropertySupportsLocalVariation(
        focusedProperty(sequencer)
    );
}

FLASHMEM void setFocusedRowValue(
...
FLASHMEM bool resetFocusedRowToDefault(
    core::state::sequencer::SequencerState& sequencer,
    uint8_t step
) {
    bool changed = false;
