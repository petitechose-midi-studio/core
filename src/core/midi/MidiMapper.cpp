...
MidiMapper::MidiMapper(IMidiOutput& midiOut, IEventBus& eventBus,
    ...
    encoder_sub_ =
        event_bus_.on(EventCategory::USER_INPUT, ENCODER_CHANGED, [this](const Event& e) {
...
MidiMapper::~MidiMapper() {
    if (encoder_sub_ != 0) { event_bus_.off(encoder_sub_); }
    if (button_sub_ != 0) { event_bus_.off(button_sub_); }
}
...
void MidiMapper::onEncoderChangedEvent(const EncoderChangedEvent& event) {
    ...
    MidiCCEvent midiEvent(config->channel, config->control, value,
                          static_cast<uint8_t>(event.encoderId));
...
void MidiMapper::onButtonPressEvent(const ButtonPressEvent& event) {
    ...
    MidiCCEvent midiEvent(config->channel, config->control, value,
                          static_cast<uint8_t>(event.buttonId));
    event_bus_.emit(midiEvent);
}
