...
MidiStudioApp::MidiStudioApp(PluginSetupFn setupPlugins)
    : display_driver_(), setup_plugins_(setupPlugins), event_bus_(), multiplexer_(),
      encoders_config_(InputFactory::createEncoders()),
      buttons_config_(InputFactory::createButtons()), display_bridge_(display_driver_),
      midi_out_(event_bus_), midi_in_(event_bus_), encoders_(encoders_config_, event_bus_),
      buttons_(buttons_config_, multiplexer_, event_bus_),
      midi_mapper_(midi_out_, event_bus_, MidiFactory::createDefault()),
      ui_(display_bridge_, event_bus_), input_manager_(encoders_, buttons_),
      ui_controller_(ui_, event_bus_), plugins_(event_bus_, midi_in_, midi_out_, encoders_, ui_) {
#ifdef DEBUG_LOGS
    waitForSerial();
#endif
}
...
bool MidiStudioApp::setup() {
    boot_complete_sub_ = event_bus_.on(EventCategory::SYSTEM, SystemEvent::BOOT_COMPLETE,
                                       [this](const Event& e) { onBootComplete(e); });

    Boot::BootManager::Components components{.displayDriver = display_driver_,
...
void MidiStudioApp::runBootSequence() {
    if (!boot_manager_.has_value()) return;
...
void MidiStudioApp::runMainLoop() {
    ...
    if (plugins_initialized_) { plugins_.update(); }
...
void MidiStudioApp::onBootComplete(const Event& event) { initializePlugins(); }
