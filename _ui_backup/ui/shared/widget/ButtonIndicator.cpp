...
ButtonIndicator::ButtonIndicator(lv_obj_t* parent, lv_coord_t size)
    : led_(nullptr), current_state_(State::OFF),
    ...
    lv_obj_set_style_bg_color(led_, lv_color_hex(BaseTheme::Color::STATUS_INACTIVE),
                              LV_STATE_DEFAULT);
...
lv_color_t ButtonIndicator::getColorForState(State state) const {
    ...
    if (custom_opacities_[idx] != 0) { return custom_colors_[idx]; }

    switch (state) {
        case State::OFF: return lv_color_hex(BaseTheme::Color::STATUS_INACTIVE);
        case State::ACTIVE: return lv_color_hex(BaseTheme::Color::STATUS_WARNING);
        case State::PRESSED: return lv_color_hex(BaseTheme::Color::STATUS_SUCCESS);
        default: return lv_color_hex(BaseTheme::Color::STATUS_INACTIVE);
    }
}
...
lv_opa_t ButtonIndicator::getOpacityForState(State state) const {
    ...
    if (custom_opacities_[idx] != 0) { return custom_opacities_[idx]; }

    switch (state) {
        case State::OFF: return LV_OPA_60;
        case State::ACTIVE: return LV_OPA_80;
        case State::PRESSED: return LV_OPA_COVER;
        default: return LV_OPA_60;
    }
}
