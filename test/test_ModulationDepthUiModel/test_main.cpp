...
namespace depth = core::ui::modulation::depth;
...
void test_scale_is_derived_from_the_canonical_source_curve() {
    ...
    const auto source = mod::createRecordedShapeModulator(graph, curves, bipolar);
    ...
    const auto* live = mod::findProjectModulationBinding(graph, assigned.bindingId);
    assert(live != nullptr);
...
void test_non_bipolar_sources_keep_the_standard_scale() {
    ...
    const auto* liveLfo = mod::findProjectModulator(graph, lfoSource.sourceId);
    ...
    const auto* liveAdsr = mod::findProjectModulator(graph, adsrSource.sourceId);
...
int main() {
    ...
    std::cout << "All ModulationDepthUiModel tests passed.\n";
    return 0;
}
