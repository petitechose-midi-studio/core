...
int main() {
    ...
    oc::diagnostics::recordPerformance({"display.lvgl.refresh", 110000, 64000, 32000});
    oc::diagnostics::recordPerformance({"main.core-state", 58000, 0, 0});
    oc::diagnostics::recordPerformance({"sequencer.timer-entry-gap", 2395, 1000, 0});
    oc::diagnostics::recordPerformance({"sequencer.playback-cc", 1966, 0, 0});
    for (uint32_t now = 4200; now < 4204; ++now) reporter.update(now);
    pump(reporter, 6200);
    assert(output.find("overflow.test samples=256") != std::string::npos);
    assert(output.find("diagnostics overflow samples=53 metrics=0") != std::string::npos);
    ...
    assert(output.find("dropped-peak label=display.lvgl.refresh elapsed=110000us") != std::string::npos);
    assert(output.find("dropped-peak label=main.core-state elapsed=58000us") != std::string::npos);
    assert(output.find("dropped-peak label=sequencer.timer-entry-gap elapsed=2395us") != std::string::npos);
    assert(output.find("dropped-peak label=sequencer.playback-cc elapsed=1966us") != std::string::npos);
