...
void logMemoryFootprint(const char*) { ++memoryReports; }
}
...
int main() {
    ...
    for (uint32_t now = 4002; now < 4006; ++now) reporter.update(now);
    reporter.update(6001);
    ...
    reporter.update(1);
    reporter.update(30001);
    assert(memoryReports == 1);
    reporter.end();
    output.clear();
