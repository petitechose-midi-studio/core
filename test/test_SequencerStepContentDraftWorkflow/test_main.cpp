...
namespace draft_workflow =
    core::handler::sequencer::step_content_draft_workflow;
...
bool rootHasChild(
    ...
    if (graph == nullptr || graph->stepNode(seq::rootStepNodeId(step)) == nullptr) {
        return false;
    }
...
struct HistoryRecorder {
    ...
    static bool canRecord(
        void* context,
        const seq::SequencerHistoryPatternChange& change
    ) {
        auto& self = *static_cast<HistoryRecorder*>(context);
        return self.admit &&
               !seq::sameMusicalHistorySnapshot(change.before, change.after);
    }

    static void recordPrepared(
        void* context,
    ...
    core::handler::SequencerHistoryDomainServices services() {
        return core::handler::SequencerHistoryDomainServices({
            .context = this,
...
void test_new_micro_is_unpublished_until_one_prepared_apply() {
    ...
    assert(seq::setActiveContentStepFromNormalized(
        sequencer,
        0,
...
void test_pristine_back_abandons_without_history() {
    ...
    const auto result = seq::openOrCreateActiveContentChild(
        sequencer,
        1,
        seq::StepContentChildKind::CYCLE_STATES,
        seq::DEFAULT_CYCLE_STATE_COUNT
    );
    assert(result.opened && result.draft);
    assert(!sequencer.stepContentDraft.modified());

    assert(draft_workflow::requestBack(sequencer) ==
...
void test_modified_back_defaults_to_save_and_supports_continue_discard() {
    ...
    sequencer.stepContentDraft.exitChoice.set(
        seq::SequencerStepContentDraftExitChoice::CONTINUE
    );
    ...
    assert(draft_workflow::requestBack(sequencer) ==
           draft_workflow::BackResult::CONTINUE_EDITING);
    sequencer.stepContentDraft.exitChoice.set(
        seq::SequencerStepContentDraftExitChoice::DISCARD
    );
    assert(draft_workflow::applyExitChoice(sequencer, tracks, history) ==
           draft_workflow::BackResult::DISCARDED);
    assert(!sequencer.stepContentDraft.active.get());
...
void test_chord_draft_sanitizes_invalid_mode_without_allocating_graph_scratch() {
    ...
    assert(seq::setAuthoringNodeChordMode(
        sequencer,
        nodeId,
        static_cast<oc::note::sequencer::StepSequencerChordMode>(0xFFU)
    ));
    ...
    assert(seq::resolveStepContentDraftChord(
        sequencer,
        nodeId,
        modePresent,
        localPresent,
