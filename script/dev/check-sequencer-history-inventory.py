...
def manifest_errors(manifest) -> list[str]:
    ...
    if len(exclusions) != 1:
        errors.append("exactly one explicit coalesced CoreState exclusion is required")
    elif exclusions[0].get("method") != "recordSequencerPatternHistory":
        errors.append("the explicit exclusion must be recordSequencerPatternHistory")
...
def collect_observation(root: Path, manifest):
    ...
    for path in source_files(root):
        ...
        if count:
            begins[relative(root, path)] = count
...
def non_member_topology_self_test(manifest) -> bool:
    ...
    exclusion_key = next(iter(drift["exclusions"]))
    drift["forwards"]["recordPattern"] += 1
    drift["adapter"][adapter_key] += 1
    drift["exclusions"][exclusion_key] += 1
    ...
    labels = (
        ...
        "explicit exclusion mismatch", "coalesced begin mismatch",
    )
...
def seam_self_test(manifest) -> bool:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for rel in seam["allowedTestUses"]:
            path = root / rel
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(f"#if defined({macro})\n#endif\n", encoding="utf-8")
