...
def persistence_lease_contract_errors(files: dict[str, str]) -> list[str]:
    ...
    for contract in (
        ...
        (project_workspace, r"sizeof\(ProjectFileBuffer\)\s*==\s*526208U", "Project file/commit buffer must remain 526,208 B on ARM"),
        ...
        (job_rpc_header, r"VERSION\s*=\s*4", "wire version changes require coordinated qualification"),
        (job_rpc_header, r"HEADER\s*=\s*24", "unified header must remain 24 bytes"),
    ...
    require(
        recovery_source,
        r"ordinary_\.advance\s*\(\s*files\s*,\s*lease_\s*,\s*scratch_\.data\s*\(\s*\)\s*,\s*scratch_\.size\s*\(\s*\)\s*\)",
        "cooperative recovery must advance ordinary journal replay with the exact lease and retained scratch",
    )
    require(
        recovery_source,
